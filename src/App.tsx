          ...
          <>
            ...
            >
              ...
              <div className="pt-6 border-t border-stone-100 dark:border-stone-800">
                <button 
                  onClick={() => signOut(auth)}
                  className="w-full flex items-center gap-3 p-3 rounded-xl text-stone-500 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-950/30 transition-colors"
                >
                  <LogOut size={20} />
...
function WritingView({ user }: { user: User }) {
  ...
  useEffect(() => {
    if (setupMode === 'countdown' && countdown !== null) {
      if (countdown > 0) {
  ...
  const displayTime = sessionType === 'stopwatch' 
    ? seconds 
    : Math.max(0, timerDuration - seconds);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-8">
          <div className="flex flex-col">
            <span className="text-xs font-bold text-stone-400 dark:text-stone-500 uppercase tracking-widest">
              {sessionType === 'stopwatch' ? 'Время' : 'Осталось'}
            </span>
            <span className="text-2xl md:text-3xl font-mono font-medium dark:text-stone-100">{formatTime(displayTime)}</span>
          </div>
          <div className="flex flex-col">
            <span className="text-xs font-bold text-stone-400 dark:text-stone-500 uppercase tracking-widest">Сл/мин</span>
            <span className="text-2xl md:text-3xl font-mono font-medium dark:text-stone-100">{wpm}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          {status === 'idle' && (
            <button 
              onClick={handleStart}
              className="flex-1 md:flex-none flex items-center justify-center gap-2 bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 px-6 py-3 rounded-xl font-semibold shadow-lg shadow-stone-200 dark:shadow-none hover:scale-105 transition-all"
            >
          ...
          <div className="absolute inset-0 z-20 bg-white/90 dark:bg-stone-950/90 backdrop-blur-sm rounded-3xl flex items-center justify-center p-6">
            <div className="max-w-sm w-full space-y-8 text-center">
              ...
              <div className="flex flex-col gap-6">
                <div className="flex items-center justify-center gap-4">
                  {[5, 15, 30, 60].map(mins => (
                    <button 
                      ...
                      className={cn(
                        "w-14 h-14 rounded-xl border font-bold transition-all",
                ...
                <div className="text-stone-400 text-sm">минут</div>
                <button 
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h2 className="text-3xl md:text-4xl font-bold dark:text-stone-100">Ваш архив</h2>
      ...
      {!loading && sessions.length > 0 && (
        <div className="bg-white dark:bg-stone-900 p-6 rounded-3xl border border-stone-200 dark:border-stone-800 shadow-sm">
          <h3 className="text-xl font-bold mb-6 dark:text-stone-100">Активность за последние 7 дней</h3>
        ...
        <div className="space-y-8">
          ...
          {allTags.length > 0 && (
            <div className="bg-white dark:bg-stone-900 p-6 rounded-3xl border border-stone-200 dark:border-stone-800 shadow-sm space-y-4">
              <h3 className="font-bold text-lg dark:text-stone-100">Облако тегов</h3>
              <div className="flex flex-wrap gap-2">
                {allTags.map(tag => (
                  <span key={tag} className="px-3 py-1 bg-stone-50 dark:bg-stone-950 text-stone-500 dark:text-stone-400 rounded-full text-sm hover:bg-stone-100 dark:hover:bg-stone-800 transition-colors cursor-default">
                    #{tag}
                  </span>
                ))}
              </div>
...
function SessionChart({ sessions }: { sessions: Session[] }) {
  ...
  const data = last7Days.map(date => {
    const daySessions = sessions.filter(s => format(new Date(s.createdAt), 'yyyy-MM-dd') === date);
    return {
  ...
  const colors = ['#f87171', '#fb923c', '#fbbf24', '#a3e635', '#4ade80', '#2dd4bf', '#38bdf8'];
        ...
        <BarChart data={data}>
              ...
              if (active && payload && payload.length) {
                return (
                  <div className="bg-white dark:bg-stone-900 p-3 rounded-xl border border-stone-200 dark:border-stone-800 shadow-xl">
                    ...
                    <p className="text-xs text-stone-500">Слов: {payload[0].payload.wordCount}</p>
                  </div>
                );
              }
              return null;
          ...
          <Bar dataKey="duration" radius={[6, 6, 0, 0]}>
            ...
            <LabelList 
              ...
              formatter={(val: number) => val > 0 ? `${val} сл` : ''}
            />
          </Bar>
        </BarChart>
      </ResponsiveContainer>
...
function FeedView() {
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h2 className="text-3xl md:text-4xl font-bold dark:text-stone-100">Лента сообщества</h2>
        <div className="flex items-center gap-2 text-stone-400 dark:text-stone-500">
          <Globe size={20} />
  ...
  return (
    ...
    >
      ...
      {isEditing ? (
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-2 p-3 bg-stone-50 dark:bg-stone-950 rounded-xl border border-stone-200 dark:border-stone-800">
            ...
            <input 
              ...
              onKeyDown={(e) => e.key === 'Enter' && addTag()}
              placeholder="Добавить тег..."
              className="flex-1 min-w-[100px] bg-transparent border-none outline-none text-xs dark:text-stone-100"
            />
          </div>
          <div className="flex justify-between items-center">
            <label className="flex items-center gap-2 cursor-pointer group/label">
              ...
              <div className={cn(
                "w-10 h-6 rounded-full p-1 transition-colors duration-300 flex items-center",
                editIsPublic ? "bg-emerald-500" : "bg-stone-200 dark:bg-stone-800"
              )}>
                <motion.div 
                  animate={{ x: editIsPublic ? 16 : 0 }}
                  className="w-4 h-4 bg-white rounded-full shadow-sm"
                />
              </div>
              <span className="text-sm font-medium text-stone-500 dark:text-stone-400 group-hover/label:text-stone-900 dark:group-hover/label:text-stone-100 transition-colors flex items-center gap-1.5">
                {editIsPublic ? <Globe size={14} /> : <Lock size={14} />}
                {editIsPublic ? "Публично" : "Приватно"}
              </span>
            </label>
            <div className="flex gap-3">
              <button onClick={() => setIsEditing(false)} className="px-4 py-2 text-sm font-medium text-stone-500">Отмена</button>
              <button onClick={handleSave} className="px-4 py-2 bg-stone-900 dark:bg-stone-100 text-white dark:text-stone-900 rounded-xl text-sm font-bold">Сохранить</button>
            </div>
          </div>
        </div>
      ) : (
        <>
          <div className={cn(
            "prose prose-stone dark:prose-invert max-w-none text-base md:text-lg leading-relaxed",
          ...
          return (
            <div 
              ...
              className={cn(
                ...
                isActive ? "bg-emerald-500 dark:bg-emerald-400 text-white dark:text-stone-900 font-bold shadow-lg shadow-emerald-200 dark:shadow-none" : "text-stone-400 dark:text-stone-500 hover:bg-stone-50 dark:hover:bg-stone-800",
