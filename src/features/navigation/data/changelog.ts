...
export const CHANGELOG: ChangelogRelease[] = [
  ...
  },
  {
    ...
    date: '2026-08-01',
    items: [
      { category: 'fix', ru: 'Отправленное сообщение видно сразу, а не только когда ИИ ответил', en: 'A sent message shows up immediately instead of only when the AI replies' },
      { category: 'fix', ru: 'Поиск по имени больше не подмешивает заметки, в которых этого имени нет', en: 'A search by name no longer mixes in notes that never mention it' },
      { category: 'improvement', ru: 'Убрана плашка «Ищет только про…»: она закрепляла период или имя из первого вопроса на весь диалог и молча отвечала на все следующие вопросы в старых рамках. Область поиска и так разбирается в каждом сообщении отдельно', en: 'Removed the "searching only …" chip: it pinned a period or name from the first question to the whole dialogue and silently answered every later question inside that frame. Scope is parsed per message anyway' },
      ...
      { category: 'fix', ru: 'Убран маркер, которым ИИ запрашивал поиск: он сбивал модель и приводил к предложению «поискать в заметках» образец из инструкции. Поиск и так выполняется перед каждым ответом', en: 'Removed the marker the AI used to request a search: it derailed the model and led to offers to "search the notes" for the example from the instruction. The search runs before every reply anyway' },
  ...
  },
  {
    ...
    items: [
      ...
      { category: 'fix', ru: 'Прикреплённая заметка в чате снова показывается свёрнутой карточкой со скрепкой (разворачивается по клику), а не сырым текстом', en: 'An attached note in the chat again shows as a collapsed paperclip card (expands on click) instead of raw text' },
      { category: 'fix', ru: 'Таблицы в ответах ИИ («Параллели с заметками» и т.п.) рендерятся как таблицы, а не строки с палочками', en: 'Tables in AI replies ("Parallels with notes" etc.) render as tables, not raw pipe rows' },
      { category: 'improvement', ru: 'Всплывающие панели в шапке чата (настройки объёма/рассуждений, выбор собеседника) стали непрозрачными — текст под ними больше не просвечивает', en: 'The chat-header popovers (volume/reasoning settings, interlocutor picker) are now opaque — messages no longer show through them' },
      { category: 'improvement', ru: 'Профиль: при открытии развёрнут только раздел «Как я пишу», остальные свёрнуты; в темах убрано «(кластеры заметок)» и «Журнал судьи» (остался в диагностике)', en: 'Profile: only "How I write" is expanded on open, the rest collapsed; the themes block drops "(note clusters)" and the judge log (kept in diagnostics)' },
      { category: 'improvement', ru: '«История моей жизни»: день показывается полным текстом со стрелкой-разворотом, а факты и инсайты выводятся отдельными списками', en: '"Story of my life": each day shows the full text with an expand arrow, and facts and insights are shown as separate lists' },
    ],
  },
  {
    version: '0.7.54',
